...
public class PushMessageReceiver extends BroadcastReceiver {
	...
	public void onReceive(final Context context, Intent intent) {
		...
		if (intent.getAction().equals(PushConstants.ACTION_MESSAGE)) {
			...
			JSONObject initJsonObject = null;
			try {
				initJsonObject = new JSONObject();
				initJsonObject.put("message", message);
			} catch (JSONException e) {
				e.printStackTrace(System.err);
			}
			
			final JSONObject finalJsonObject = initJsonObject;
			new Thread() {
				@Override
				public void run() {
					...
					PushDemoActivity.messageCbContext.sendPluginResult(result);
				}
			}.start();
		} else if (intent.getAction().equals(
				"com.baidu.android.pushservice.action.USERNAME")) {
			...
			Log.d(TAG, "className=" + className);
				...
				"com.baidu.android.pushservice.action.TAGS")) {
			...
			if (method.equalsIgnoreCase(METHOD_SET_TAG)) {
				if (errorCode == 0) {
					...
					if (urlString.contains("http")) {
						...
						new Thread() {
							...
							public void run() {
								// 你要执行的方法
								String recvMessageString = HttpRequest
										.sendPost(urlString, paraString);
								Log.d(TAG, "callback : " + recvMessageString);
								if (recvMessageString == "") {
									// 返回给JS调用
									PushDemoActivity.cbContext
											.error(ErrorPush.NET_ERROR
													.toString());
								} else
				...
				if (errorCode == 0) {
					...
					if (urlString.contains("http")) {
						...
						final String paraString = String.format(
								...
								paramPost);
						// String recvMessageString=sendPost(urlString,
						// paraString);
						new Thread() {
							@Override
							public void run() {
				...
				if (errorCode == 0) {
					...
					try {
						...
						if (urlString.contains("http")) {
							...
							PushDemoActivity.cbContext.success(jsonParaString);
					} catch (JSONException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
				...
				if (errorCode == 0) {
					...
					JSONObject paramJsonObject = null;
					try {
						paramJsonObject = getJSONObject(listJsonObject
								.getString("response_params"));
					...
					try {
						...
						for (int i = 0; i < listTags.length(); i++) {
							...
							if (TagsListCallback.length() == 0)
								...
								TagsListCallback = TagsListCallback + ","
